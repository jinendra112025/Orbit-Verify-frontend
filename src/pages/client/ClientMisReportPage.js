...
import { Paper, Box, Typography, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, CircularProgress, TablePagination } from '@mui/material';
...
const convertCasesToCSV = (data) => {
    const headers = ['Candidate Name', 'Case Create Date', 'Case Start Date', 'Report Closed Date', 'Status', 'TAT (Days)'];
    ...
    useEffect(() => {
        const fetchReport = async () => {
            ...
            try {
                ...
                const response = await api.get(`/cases?page=${page + 1}&limit=${rowsPerPage}`);
